          ...
          <CardHeader className="flex-shrink-0 border-b">
            <div className="flex items-center justify-between">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1">
                ...
                <Button variant={userType === 'founder' ? 'default' : 'ghost'} size="sm" onClick={() => setUserType('founder')} className="text-xs">
                  As Founder
                </Button>
              </div>
            </div>
          ...
          <CardContent className="flex-1 flex flex-col p-0">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 ? <div className="text-center text-gray-500 mt-8">
                ...
                </div> : messages.map(msg => <div key={msg.id} className={`flex ${msg.sender_type === userType ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${msg.sender_type === userType ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white' : 'bg-gray-100 text-gray-900'}`}>
                      <div className="flex items-center space-x-2 mb-1">
                        <span className={`text-xs font-medium ${msg.sender_type === userType ? 'text-purple-100' : 'text-gray-600'}`}>
                          {msg.sender_type === 'founder' ? 'Founder' : 'Funder'}
                        </span>
                        {msg.sender_wallet_address && <span className={`text-xs font-mono ${msg.sender_type === userType ? 'text-purple-200' : 'text-gray-500'}`}>
                            {msg.sender_wallet_address.substring(0, 6)}...{msg.sender_wallet_address.substring(38)}
                          </span>}
                      </div>
                      <p className="text-sm">{msg.message_content}</p>
                      <p className={`text-xs mt-1 ${msg.sender_type === userType ? 'text-purple-100' : 'text-gray-500'}`}>
            ...
            <div className="border-t p-4">
              <div className="flex space-x-2">
                <Input value={message} onChange={e => setMessage(e.target.value)} placeholder="Type your message..." onKeyPress={handleKeyPress} className="flex-1" />
