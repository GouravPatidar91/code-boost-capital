...
const Index = () => {
  ...
  const { allFundingData, loading: fundingLoading } = useFundingData();

  const getFundingDataForStartup = (startupId: string) => {
    return allFundingData.find(data => data.startup_id === startupId);
  };

  // Calculate total platform stats from real data
  const totalFunded = allFundingData.reduce((sum, data) => sum + data.total_raised, 0);
  const totalFunders = allFundingData.reduce((sum, data) => sum + data.total_funders, 0);
          ...
          {loading || fundingLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {startups.map((startup) => {
                const fundingData = getFundingDataForStartup(startup.id);
      ...
      <section className="py-16 bg-gradient-to-r from-purple-600 to-blue-600 text-white">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              <div className="text-4xl font-bold mb-2">${totalFunded.toLocaleString()}</div>
            ...
            <div>
              <div className="text-4xl font-bold mb-2">{totalFunders}</div>
